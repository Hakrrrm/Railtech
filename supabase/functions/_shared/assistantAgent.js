...
Speak in concise, calm, friendly operational English. Lead with the useful answer. Use V12-style display IDs, Singapore dates/times and km. Avoid hype, emojis, claiming certainty about forecasts, and unnecessary jargon. Clarify whether "longer" means time until due or workshop duration when ambiguous.
...
If a proposal is already pending, discuss it or tell the operator to confirm/discard it first. On tool errors, explain the limitation and do not claim success. Use short paragraphs or compact bullets, normally under180 words.`
...
export function hasSchedulingIntent(message, history = []) {
  ...
  if (/^(yes|yes please|go ahead|go ahead please|proceed|do it)[.!\s]*$/.test(text)) {
    ...
    return /\b(propos|schedule|booking|plan)\b/i.test(previous)
  }
  ...
  if (/^(auto[- ]?schedule|schedule|book)\b/.test(command)) return /\b(v\d+|d\d+|lrv|lrvs|vehicle|vehicles|fleet|maintenance|repairs?|faults?|due|these|them|those|all|it|plan|slots?)\b/.test(command)
  return /^(propose|create|prepare|add|plan)\b/.test(command)
    && /\b(maintenance|schedule|bookings?|slots?|lrvs?|v\d+|d\d+|this plan|the plan)\b/.test(command)
}
...
export async function runAssistantTurn({ message, history = [], loadData, saveProposal, provider, now = new Date(), model = ASSISTANT_MODEL, pendingBatch = null, planningPreferences = null }) {
  ...
  for (let round = 0; round < 4; round += 1) {
    ...
    const actionTool = round === 1 && (canPropose || needsPreview) ? (canPropose ? 'propose_schedule' : 'preview_schedule') : null
    const roundTools = actionTool ? tools.filter(t => [actionTool, 'ask_clarification'].includes(t.name)) : tools
    const response = await provider({ model, instructions: undefined, input, tools: roundTools, parallel_tool_calls: false,
      tool_choice: round === 0 ? { type: 'function', name: 'get_fleet_status' } : actionTool ? 'required' : 'auto',
    ...
    for (const call of toolCalls) {
      ...
      try {
        if (!allowed.has(call.name)) throw new Error('This action is not available. Confirm proposals using the operator button.')
        ...
        if (call.name === 'get_fleet_status') {
          ...
          validatedPreferences = validateConstraints(args, now)
          // Re-read on every preview/write: prior conversational data never authorizes a write.
          data = await loadData()
          plan = buildAssistantPlan(data, args, now)
          if (call.name === 'propose_schedule') {
            ...
            result = { ...plan, batch, instruction: 'Blue proposals only. Operator must use Confirm proposal.' }
...
export function proposalText(plan, batch) {
  if (!batch) return `No bookings were added. ${plan.summary || 'No eligible slots matched this request.'} ${(plan.skipped || []).map(s => `${s.lrvId}: ${s.reason}`).join(' ')}`
  const count = plan.bookings.length
  return `I've added ${count} proposed booking${count === 1 ? '' : 's'} in blue. Review the slots below, then use Confirm proposal to commit them. ${plan.summary || ''}${plan.skipped?.length ? ` ${plan.skipped.length} LRV(s) could not be scheduled; the reasons are listed below.` : ''}`
}
