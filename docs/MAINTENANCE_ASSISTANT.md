...
**Current demo access:** explicitly enabled for the synthetic fleet. Each browser uses a random session capability; only its hash is stored on the server. All new assistant tables and RPCs are inaccessible to anonymous browser roles. The existing application still exposes its legacy anonymous manual scheduling/reset RPCs, so this deployment is a demo, not a fully authenticated operational control system.
