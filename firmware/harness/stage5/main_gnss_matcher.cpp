 ...
 *   - Qwiic I2C pins are unconfirmed (-1) in pins_board.h; the IMU
 *     silently stays disabled until a human fills them in from the
 *     schematic, even with IMU_ENABLED=1.
...
static void gnss_matcher_task(void *arg)
{
    ...
    if (!imu_ready) {
        Serial.println("[imu] disabled: init failed or QWIIC_I2C_*_PIN unconfirmed (-1) in pins_board.h");
    }
#endif
