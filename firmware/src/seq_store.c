 ...
 * Backed by a plain text file ("seq odo_m\n") so a "reboot" can be
...
#define SEQ_STORE_KEY_ODO_M "odo_m"
...
void seq_store_init(void)
{
    ...
    int64_t odo_val = 0;
    if (nvs_get_i64(s_handle, SEQ_STORE_KEY_ODO_M, &odo_val) == ESP_OK) {
        s_odo_m = odo_val;
    }
}
...
int seq_store_commit(uint32_t new_seq, int64_t new_odo_m)
{
    ...
    if (nvs_set_i64(s_handle, SEQ_STORE_KEY_ODO_M, new_odo_m) != ESP_OK) {
        return -1;
    }
