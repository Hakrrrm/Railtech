...
import { cycleLabel, forecastLabel, formatDateTime, formatKm, qualityLabel, statusLabel } from '../lib/format'
  ...
  return <>
    ...
    <DataBoundary loading={state.loading} error={state.error} empty={!data?.summary} onRetry={state.refresh}>
      {data?.summary && <>
        ...
        <Card title="Maintenance log" eyebrow="Definite mileage recorded at completion">
          {data.events.length ? <div className="table-wrap"><table><thead><tr><th>Completed</th><th>Cycle</th><th>Physical reading</th><th>Reset cycles</th><th>Technician</th><th>Notes</th></tr></thead><tbody>
            {data.events.map((event) => <tr key={event.id}><td>{formatDateTime(event.completed_at)}</td><td><strong>{cycleLabel(event.primary_cycle)}</strong></td><td>{formatKm(event.completion_mileage_km, 1)}</td><td>{event.reset_cycles.map(cycleLabel).join(' · ')}</td><td>{event.technician_id}</td><td>{event.notes || statusLabel(event.source)}</td></tr>)}
