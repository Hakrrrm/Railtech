...
function buildEvidence(data, filter, search) {
  ...
  const events = data.events.map((row) => ({ key: `m-${row.id}`, type: 'maintenance', lrvId: row.lrv_id, time: row.completed_at, actor: row.technician_id, title: `${row.lrv_id} completed ${cycleLabel(row.primary_cycle)}`, description: `${formatKm(row.completion_mileage_km, 1)} definite reading; reset ${row.reset_cycles.map(cycleLabel).join(', ')}. ${row.notes || ''}`, badge: 'Completed' }))
