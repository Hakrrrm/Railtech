...
export function assistantTools(canPropose) {
  return [
    ...
    tool('preview_bay_clearance', 'Calculate options to clear the current resolved source bay/date. Selects its bookings from the database, read-only. Use only for an unrestricted request to clear that bay; for specific destination/date/time constraints use reschedule tools.', empty),
    ...(canPropose ? [tool('propose_bay_clearance', 'Prepare blue moves to clear the current resolved source bay/date, selecting only its actual bookings. Use for an unrestricted clearance request; use propose_reschedule for specific destination/date/time constraints. Operator confirms separately.', empty)] : []),
...
For clearing a bay or rescheduling ALL work in a source bay on a date, set sourceBayId to its exact configured ID and sourceDate to YYYY-MM-DD. Distinguish source from destination: "move V24 to Bay 2" does NOT make Bay 2 the source. Retain the previous source only while the user continues that request. Clear both fields when changing topic. Resolve dates (including 21 Sept, weekdays, tomorrow) using the supplied Singapore date and conversation; do not silently invent a date. Missing bay/date requires clarify, retaining whichever field is known. Interpret explicit date/bay corrections as changes, not acceptance of the old plan.
...
Previous preferences: ${JSON.stringify(planningPreferences)}.` },
    ...
    tools: [tool('interpret_request', 'Resolve conversational intent and source scope.', {
      type: 'object', additionalProperties: false, properties: {
        ...
        sourceBayId: { type: ['string','null'] }, sourceDate: { type: ['string','null'] },
  ...
  if (calls.length !== 1 || calls[0].name !== 'interpret_request' || !decision
    ...
    || (decision.sourceBayId !== null && !data.bays.some(b => b.bay_id === decision.sourceBayId))) throw new Error('Could not resolve this planning request safely. Please try again; no proposal was saved.')
    ...
    for (const call of toolCalls) {
      ...
      try {
        ...
        if (call.name === 'get_fleet_status') {
          ...
          return { text: rows.length ? previewText(plan) : 'This bay is already clear on the requested date. No rescheduling is needed.', plan, batch: null, audit: [...audit, { tool: call.name, count: plan.bookings.length }], usage, planningPreferences: { ...validatedPreferences, reviewedPreview } }
