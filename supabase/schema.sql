...
begin
  ...
  select value_km into v_last_anchor
  from mileage_anchors
  where lrv_id = p_lrv_id and superseded_by is null
  order by ts desc, id desc limit 1;
  if v_last_anchor is not null and p_value_km < v_last_anchor then
    raise exception 'Reading cannot be below the latest accepted physical reading (%)', v_last_anchor;
  end if;

  select odo_km into v_device_odo
  from segment_traversals
  where lrv_id = p_lrv_id and ts <= now() + interval '10 minutes'
  order by ts desc, id desc limit 1;
  v_divergence := case when v_device_odo is null then null else p_value_km - v_device_odo end;

  insert into mileage_anchors (
    lrv_id, ts, technician_id, source, value_km, override, override_reason,
    image_uri, gnss_odo_km, divergence_km
  ) values (
    p_lrv_id, now(), btrim(p_technician_id), 'technician_ocr', p_value_km,
    abs(coalesce(v_divergence, 0)) >= 50,
    case when abs(coalesce(v_divergence, 0)) >= 50
      then 'Physical/device divergence requires evidence review'
    end,
    btrim(p_image_uri), v_device_odo, v_divergence
  ) returning id into v_anchor_id;

  insert into technician_observations (
    lrv_id, booking_id, anchor_id, technician_id, image_uri,
    ocr_value_km, ocr_confidence, reviewed_manually, submitted_value_km
  ) values (
    p_lrv_id, p_booking_id, v_anchor_id, btrim(p_technician_id), btrim(p_image_uri),
...
begin
  ...
  where bucket_id = 'hubometer-evidence'
    and (storage.foldername(name))[1] ~ '^D(0[1-9]|[12][0-9]|30)$';
